<x-app-layout title="Edit Destinasi Wisata">
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-2xl text-slate-800 leading-tight">
                    {{ __('Edit Destinasi Wisata') }}
                </h2>
                <p class="text-sm text-slate-500 mt-1">Ubah data wisata: {{ $wisata->nama_wisata }}</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama_wisata" id="nama_wisata" value="{{ old('nama_wisata', $wisata->nama_wisata) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm @error('nama_wisata') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @enderror" required>
                                @error('nama_wisata')
                            ...
                            <div>
                                ...
                                <select name="kategori" id="kategori" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm @error('kategori') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @enderror" required>
                                    ...
                                    <option value="Buatan" {{ old('kategori', $wisata->kategori) === 'Buatan' ? 'selected' : '' }}>Wisata Buatan</option>
                                    <option value="Budaya" {{ old('kategori', $wisata->kategori) === 'Budaya' ? 'selected' : '' }}>Wisata Budaya & Sejarah</option>
                                </select>
                                @error('kategori')
                                    <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" id="deskripsi" rows="5" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm @error('deskripsi') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @enderror" required>{{ old('deskripsi', $wisata->deskripsi) }}</textarea>
                                @error('deskripsi')
                                    <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @if ($wisata->foto)
                                    <div class="mb-4">
                                        ...
                                        <div class="relative w-48 h-32 rounded-xl overflow-hidden border border-slate-100 shadow-sm">
                                            <img src="{{ asset('storage/' . $wisata->foto) }}" class="w-full h-full object-cover" alt="{{ $wisata->nama_wisata }}">
                                        </div>
                                    </div>
                                @endif
                                    ...
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-slate-600 justify-center">
                                            <label for="foto" class="relative cursor-pointer bg-white rounded-md font-medium text-emerald-600 hover:text-emerald-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-emerald-500">
                                                <span >Unggah file foto baru</span>
                                                <input id="foto" name="foto" type="file" accept="image/*" class="sr-only">
                                            </label>
                                        </div>
                                        <p class="text-xs text-slate-400">PNG, JPG, JPEG, WEBP hingga 2MB (Biarkan kosong jika tidak ingin diubah)</p>
                                    </div>
                                </div>
                                @error('foto')
                                    <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', $wisata->latitude) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-mono @error('latitude') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @enderror" required>
                                    @error('latitude')
                                        <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', $wisata->longitude) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm font-mono @error('longitude') border-rose-500 focus:ring-rose-500 focus:border-rose-500 @enderror" required>
                                    @error('longitude')
                                        <p class="text-rose-500 text-xs mt-1.5">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <span class="text-xs text-slate-400 flex items-center gap-1.5">
                                    ...
                                    Geser peta dan klik untuk memindahkan koordinat pin lokasi wisata. Pin juga dapat digeser secara manual.
                                </span>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Map Initialization (Pre-set coordinates from Wisata model)
            ...
            // Create marker initialized at the existing coordinates
